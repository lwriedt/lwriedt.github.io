...
Our minds and thoughts are influenced by messages, social media, news feeds and everywhere we are exposed to the endless stream of information. Much of this is sorted, directed and even generated by systems build on algorithms and artificial intelligence.

As a society, we need to benefit from the enormous potential while not being naive optimists. Like all powerful new technologies, algorithms combined with big data must be controlled. We need to realize, that when decision power moves from man to machines, risks of misuse moves as well.
...
There are no fixed measures that validate a certain use of data as ethical. In general, most cases building on consent from the data owner or subject is considered in the ethical realm. 
...
When decision power is transferred from humans to systems, there is a motivation to also corrupt machines and AI algorithms now in charge.
